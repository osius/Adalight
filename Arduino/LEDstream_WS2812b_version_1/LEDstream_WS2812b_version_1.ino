...
//
//  This version uses the LIB Adafruit_NeoPixel to work with the WS2812b LEDs.
//  
//  This Version works with a 60LED-Stripe with a  frames/sec: 52, bytes/sec: 9672
//    tested with the programme colorswirl (see the C folder) and 
//    an Arduino UNO. 
//
//  TOOD:
//   - Cleanup: remove the SPI code
//   - Show a startup Pattern
//   - remove flicker when sending more the 59 frames/sec.
//
// --------------------------------------------------------------------
...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(LED_NR, LED_DDR, NEO_GRB + NEO_KHZ800);
...
//  reset();
  ...
  strip.setPixelColor(strip.numPixels()-1, 100,100,100);
  strip.show();
  //delay(1000); 
  //reset();
  //delay(1000); 
  ...
  //Serial.print(3L * (256L * (long)0 + (long)10 + 1L));  
  //Serial.print("\n"); 
  startTime    = micros();
...
//            SPDR = buffer[indexOut++];   // Issue next byte
            ...
            strip.setPixelColor(ledcount++, buffer[indexOut++], buffer[indexOut++],buffer[indexOut++]);
